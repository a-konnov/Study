...
namespace EnhancedUI.EnhancedScroller {
    ...
    public class EnhancedScroller : MonoBehaviour {
        ...
        /// <summary>
        /// Whether the scroller controllerd by drag
        /// </summary>
        [SerializeField]
        ...
        /// <summary>
        /// Need special sorting and shift to show central item over others 
        /// </summary>
        public bool isCoverflowViewMode;
            ...
            set {
                ...
                if (_scrollPosition != value) {
                    ...
                    if (scrollDirection == ScrollDirectionEnum.Vertical) {
                        ...
                        scrollRect.verticalNormalizedPosition = 1f - (_scrollPosition / ScrollSize);
                    } else {
        ...
        public float ScrollSize {
            get {
                if (scrollDirection == ScrollDirectionEnum.Vertical)
                    return _container.rect.height - scrollRectTransform.rect.height;
                else
                    return _container.rect.width - scrollRectTransform.rect.width;
            }
        }
        ...
        public float TweenTimeLeft {
            get {
                if (IsTweening)
                    return _tweenTime - _tweeningTime;
                return 0;
            }
        }
        ...
        public void ReloadData(float scrollPositionFactor = 0) {
            ...
            if (scrollRect == null || scrollRectTransform == null || _container == null) {
                _scrollPosition = 0f;
                return;
            }
        ...
        public List<EnhancedScrollerCellView> GetActiveCelllViews() {
            List<EnhancedScrollerCellView> activeViews = new List<EnhancedScrollerCellView>();
            for (var i = 0; i < _activeCellViews.Count; i++) {
                activeViews.Add(_activeCellViews[i]);
            }
            return activeViews;
        }
        ...
        public void ClearActive() {
            for (var i = 0; i < _activeCellViews.Count; i++) {
                Destroy(_activeCellViews[i].gameObject);
            }
        ...
        public void ClearRecycled() {
            for (var i = 0; i < _recycledCellViews.Count; i++) {
                Destroy(_recycledCellViews[i].gameObject);
            }
        ...
        public void JumpToDataIndex(int dataIndex,
            ...
            float tweenTime = 0f,
            bool ignoreIfVisible = true,
            bool checkIsFullyVisible = false,
            Action jumpComplete = null
        ) {
            // ignore the jump if the active cell range is entirely inside the scroller's visible area
            bool isNeedIgnore = ignoreIfVisible && StartDataIndex == 0 && EndDataIndex == (NumberOfCells - 1);
            if (isNeedIgnore) {
                if (checkIsFullyVisible) {
                    bool isScrollingForward = ScrollPosition + ScrollRectSize < _cellViewOffsetArray[dataIndex];
                    float cellEdgePos  = isScrollingForward ? _cellViewOffsetArray[dataIndex] : _cellViewOffsetArray[dataIndex] - _cellViewSizeArray[dataIndex];
                    float visibleEdge = ScrollPosition + (isScrollingForward ? ScrollRectSize : 0);
                    isNeedIgnore = isScrollingForward ? cellEdgePos <= visibleEdge : cellEdgePos >= visibleEdge;
                }
            }
        ...
        private void TryAutoSnap() {
            ...
            // don't scroll when user scrolls
            if (scrollRect is ScrollRectEx && (scrollRect as ScrollRectEx).IsDragging)
                return;
        ...
        public void Snap() {
            ...
            JumpToDataIndex(_snapDataIndex, snapJumpToOffset, snapCellCenterOffset, snapUseCellSpacing, snapTweenType, snapTweenTime, false, jumpComplete: SnapJumpComplete);
        }
        ...
        private void _CalculateCurrentActiveCellRange(out int startIndex, out int endIndex) {
            ...
            var endPosition = _scrollPosition + (scrollDirection == ScrollDirectionEnum.Vertical ? scrollRectTransform.rect.height : scrollRectTransform.rect.width);
        ...
        /// <summary>
        /// Ugly sorting for coverflow views in UI
        /// </summary>
        private void _TryApplyCoverflowSorting() {
            if (!isCoverflowViewMode)
                return;
            
            float cellSize = (_delegate != null ? _delegate.GetCellViewSize(this, 0) : 0);
            int centralDataIdx = Mathf.RoundToInt(_scrollPosition / cellSize);
            int siblingIdx = _activeCellViews.Count;
            
            var dataToSibling = new Dictionary<int, int> {
                { centralDataIdx, siblingIdx }
            };
            siblingIdx--;
            
            bool needExpandIndexes = true;
            int upperDataIdx = centralDataIdx - 1;
            int lowerDataIdx = centralDataIdx + 1;
            
            while (needExpandIndexes) {
                needExpandIndexes = false;
                
                if (upperDataIdx >= _activeCellViewsStartIndex) {
                    dataToSibling.Add(upperDataIdx, siblingIdx);
                    
                    upperDataIdx--;
                    siblingIdx--;
                    needExpandIndexes = true;
                }
                if (lowerDataIdx <= _activeCellViewsEndIndex) {
                    dataToSibling.Add(lowerDataIdx, siblingIdx);
            ...
            var sortedDataIdxes = dataToSibling.OrderBy(ds => ds.Value).Select(ds => ds.Key).ToList();
            
//            var activeDataIndexes = new List<int>();
//            for (int i = 0; i < _activeCellViews.Count; i++) {
//                activeDataIndexes.Add(_activeCellViews[i].dataIndex);
//            }
//            Log.Info(" > centralDataIdx: "+ centralDataIdx
//                +", active cells datas: "+ activeDataIndexes.ToDebugString()
//                +"; sorted data indexes: "+ sortedDataIdxes.ToDebugString()
//                +"; data->sib: "+ dataToSibling.ToDebugString());
            
            for (int i = 0; i < sortedDataIdxes.Count; i++) {
                int sibIndex = i + 1;
                int dataIdx = sortedDataIdxes[i];
                
                var cell = CellByDataIndex(dataIdx);
                
                cell.transform.SetSiblingIndex(sibIndex);
                
                int dataIdxLinearPos = DataIndexArrayPos(dataIdx);
                float adaptShift = (sibIndex -1 - dataIdxLinearPos) *cellSize;
                cell.SetCoverflowShift(adaptShift);
            }
        }

        private EnhancedScrollerCellView CellByDataIndex(int dataIndex) {
            for (int i = 0; i < _activeCellViews.Count; i++) {
        ...
        private int DataIndexArrayPos(int dataIndex) {
            for (int i = 0; i < _activeCellViews.Count; i++) {
                if (_activeCellViews[i].dataIndex == dataIndex)
                    return i;
            }
        ...
        private void Init() {
            ...
            {
                GameObject go = new GameObject("Last Padder", typeof(RectTransform), typeof(LayoutElement));
                go.transform.SetParent(_container, false);
                _lastPadder = go.GetComponent<LayoutElement>();
            }
            // create the recycled cell view container
            {
                GameObject go = new GameObject("Recycled Cells", typeof(RectTransform));
        ...
        void OnEnable() {
            ...
            if (scrollRect is ScrollRectEx) {
                (scrollRect as ScrollRectEx).OnDragReleased += OnScrollerDragReleased;
            }
        }
        ...
        private void _ScrollRect_OnValueChanged(Vector2 val) {
            _scrollPosition = (scrollDirection == ScrollDirectionEnum.Vertical)
                ? (1f - val.y) * ScrollSize
                : val.x * ScrollSize;
        ...
        private void OnScrollerDragReleased() {
        ...
        IEnumerator TweenPosition(TweenType tweenType, float time, float start, float end, Action tweenComplete) {
            if (tweenType == TweenType.immediate || time == 0) {
                ...
                while (_tweeningTime < time) {
                    switch (tweenType) {
                        case TweenType.linear: newPosition = linear(start, end, (_tweeningTime / time)); break;
                        case TweenType.spring: newPosition = spring(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInQuad: newPosition = easeInQuad(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutQuad: newPosition = easeOutQuad(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutQuad: newPosition = easeInOutQuad(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInCubic: newPosition = easeInCubic(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutCubic: newPosition = easeOutCubic(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutCubic: newPosition = easeInOutCubic(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInQuart: newPosition = easeInQuart(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutQuart: newPosition = easeOutQuart(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutQuart: newPosition = easeInOutQuart(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInQuint: newPosition = easeInQuint(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutQuint: newPosition = easeOutQuint(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutQuint: newPosition = easeInOutQuint(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInSine: newPosition = easeInSine(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutSine: newPosition = easeOutSine(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutSine: newPosition = easeInOutSine(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInExpo: newPosition = easeInExpo(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutExpo: newPosition = easeOutExpo(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutExpo: newPosition = easeInOutExpo(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInCirc: newPosition = easeInCirc(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutCirc: newPosition = easeOutCirc(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutCirc: newPosition = easeInOutCirc(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInBounce: newPosition = easeInBounce(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutBounce: newPosition = easeOutBounce(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutBounce: newPosition = easeInOutBounce(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInBack: newPosition = easeInBack(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutBack: newPosition = easeOutBack(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutBack: newPosition = easeInOutBack(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInElastic: newPosition = easeInElastic(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeOutElastic: newPosition = easeOutElastic(start, end, (_tweeningTime / time)); break;
                        case TweenType.easeInOutElastic: newPosition = easeInOutElastic(start, end, (_tweeningTime / time)); break;
                    }
                    ...
                    _tweeningTime += Time.unscaledDeltaTime;
            ...
            // reset tween variabless before fire jump complete callback
            IsTweening = false;
            _tweenTime = 0;
